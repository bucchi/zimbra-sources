...
)
{
    ngx_str_t                    pdu;
    mc_work_t                    w;
    size_t                       l,ttl,ttld,newl,newd,i,j,k,base;
    u_char                      *p;
    ...
    if ((user.len == alias.len) &&
        (ngx_memcmp(user.data, alias.data, alias.len) == 0)
       )
    {
    	 /* bug 66469, take the part before @ as the alias */
		for(i = 0; i < alias.len; i++) {
			if(alias.data[i] == '@') {
				//trim the '@domain' part in s->key_alias
				base = sizeof("alias:user=") - 1 + i;
				k = s->key_alias.len - (sizeof("alias:user=") - 1) - alias.len;
				for (j = 0; j < k; j++) {
					s->key_alias.data[j + base] = s->key_alias.data[j + base + (alias.len - i)];
				}
				s->key_alias.len -= (alias.len - i);
				break;
			}
		}

		if (i == alias.len) { /* not found */
			w.on_failure (&w);
			return 0;
		}
    }

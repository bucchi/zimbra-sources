...
static ngx_int_t ngx_http_upstream_do_init_zmauth_peer(ngx_http_request_t *r,
        ngx_http_upstream_srv_conf_t *us, ngx_http_upstream_stage_pt stage,
        enum ngx_http_zmauth_type type);
...
ngx_http_upstream_init_zmauth_peer(ngx_http_request_t *r,
        ngx_http_upstream_srv_conf_t *us, ngx_http_upstream_stage_pt stage)
{
    return ngx_http_upstream_do_init_zmauth_peer(r, us, stage, zmauth_web_client);
}
...
ngx_http_upstream_init_admin_zmauth_peer(ngx_http_request_t *r,
        ngx_http_upstream_srv_conf_t *us, ngx_http_upstream_stage_pt stage)
{
    return ngx_http_upstream_do_init_zmauth_peer(r, us, stage, zmauth_admin_console);
}
        ...
        enum ngx_http_zmauth_type type)
{
    ...
    if (ngx_http_upstream_init_round_robin_peer(r, us,
            ngx_http_upstream_noop_zmauth_stage) != NGX_OK) {
...
zmauth_lookup_result_handler(ngx_zm_lookup_work_t * work) {
    ...
    /* bug 64775, 62374, we have to invoke "stage" at last statement */
    ctx->stage(r);
}
